...
import type { ImportFieldKey, ImportTemplateColumn } from "@/features/products/types";
import {
  Label,
  Select,
  SelectContent,
    ...
    <div className="space-y-3">
      <div className="space-y-1">
        <h3 className="text-sm font-semibold text-foreground">
          {t("import.upload.mappingTitle")}
        </h3>
        <p className="text-sm text-muted-foreground">
          {t("import.upload.mappingDescription")}
        </p>
      </div>

      <ImportSheetScroller label={t("import.upload.mappingTitle")}>
        <table className="min-w-max border-collapse text-sm">
          ...
          <tbody>
            <tr>
              {headers.map((header, index) => {
                const selectId = `${formId}-col-${index}`;
                      ...
                      <Select
                        ...
                        onValueChange={(next) => {
                          const key =
                            next === IGNORE_VALUE
                              ? null
                              : (next as ImportFieldKey);
                          onMappingChange(
                            assignColumnMapping(mapping, index, key),
                          );
                        }}
                        disabled={disabled}
                      >
