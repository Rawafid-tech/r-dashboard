...
import { createAdminPlan } from "@/features/admin/plans/api/admin-plans.api";
import type { CreatePlanRequest } from "@/features/admin/plans/types";
...
export function useCreatePlan() {
  ...
  return useMutation({
    mutationFn: (payload: CreatePlanRequest) => createAdminPlan(payload),
    onSuccess: () => {
    ...
    onError: (error) => {
      ...
      if (isApiError(error, 409)) {
        toast.error(t("plans.toast.duplicateCode"));
        return;
      }
