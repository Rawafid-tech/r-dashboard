...
export function WalletEmptyState({ hasFilters }: WalletEmptyStateProps) {
  const { t } = useTranslation("wallet");
  const Icon = hasFilters ? SearchX : Receipt;
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-4 text-base font-semibold text-foreground">
        {hasFilters ? t("empty.filterTitle") : t("empty.title")}
      </h3>
      <p className="mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">
        {hasFilters ? t("empty.filterDescription") : t("empty.description")}
      </p>
    </div>
  );
}
