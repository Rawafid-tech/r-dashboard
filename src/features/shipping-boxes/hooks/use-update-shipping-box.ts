...
import { updateShippingBox } from "@/features/shipping-boxes/api/shipping-boxes.api";
...
export function useUpdateShippingBox(boxId: string) {
  const queryClient = useQueryClient();
  const { t } = useTranslation("shippingBoxes");
  const { t: tCommon } = useTranslation("common");

  return useMutation({
    mutationFn: (payload: ShippingBoxPayload) =>
      updateShippingBox(boxId, payload),
    onSuccess: (box) => {
